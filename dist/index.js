    ...
    function findInPath(tool2) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const directory of directories) {
          const filePath = yield ioUtil.tryGetExecutablePath(path2.join(directory, tool2), extensions);
          if (filePath) {
  ...
  "node_modules/glob/common.js"(exports2) {
    ...
    function setopts(self3, pattern, options) {
      ...
      if (!ownProp(options, "cwd"))
        ...
        self3.cwd = path2.resolve(options.cwd);
        self3.changedCwd = self3.cwd !== cwd;
      }
      self3.root = options.root || path2.resolve(self3.cwd, "/");
      self3.root = path2.resolve(self3.root);
    ...
    function makeAbs(self3, f2) {
      var abs = f2;
      if (f2.charAt(0) === "/") {
        abs = path2.join(self3.root, f2);
      } else if (isAbsolute(f2) || f2 === "") {
        abs = f2;
      } else if (self3.changedCwd) {
        abs = path2.resolve(self3.cwd, f2);
      } else {
    ...
    function isIgnored(self3, path3) {
      if (!self3.ignore.length)
        return false;
    ...
    function childrenIgnored(self3, path3) {
      if (!self3.ignore.length)
        return false;
      return self3.ignore.some(function(item) {
...
var shouldExcludePath = (path2, pathsToIgnore, globsToIgnore) => {
  if (!path2)
    return false;
  const processedPath = processPath(path2);
  ...
  return globsToIgnore.some((glob) => {
    ...
    return (0, import_micromatch.isMatch)(processedPath, glob, {
      dot: true,
      matchBase: true
    });
  });
...
var processDir = async (rootPath = "", excludedPaths = [], excludedGlobs = []) => {
  ...
  const getFileStats = async (path2 = "") => {
    const stats = await import_fs.default.statSync(`./${path2}`);
    const name = path2.split("/").filter(Boolean).slice(-1)[0];
    const size = stats.size;
    const relativePath = path2.slice(rootPath.length + 1);
    return {
      name,
      path: relativePath,
      size
  ...
  const addItemToTree = async (path2 = "", isFolder = true) => {
    try {
      console.log("Looking in ", `./${path2}`);
      if (isFolder) {
        const filesOrFolders = await import_fs.default.readdirSync(`./${path2}`);
        const children2 = [];
        for (const fileOrFolder of filesOrFolders) {
          const fullPath = nodePath.join(path2, fileOrFolder);
        ...
        const stats2 = await getFileStats(path2);
        return { ...stats2, children: children2 };
      }
      if (shouldExcludePath(path2, fullPathFoldersToIgnore, excludedGlobs)) {
        return null;
      }
      const stats = getFileStats(path2);
      return stats;
    } catch (e3) {
      console.log("Issue trying to read file", path2, e3);
      return null;
    }
  };
...
var processChild = (child, getColor, cachedOrders, i = 0, fileColors) => {
  ...
  if ((children2 == null ? void 0 : children2.length) === 1) {
    ...
    path2 = children2[0].path;
    children2 = children2[0].children;
  }
  const pathWithoutExtension = path2 == null ? void 0 : path2.split(".").slice(0, -1).join(".");
...
var main = async () => {
  ...
  try {
    const configPath = import_path2.default.join(process.cwd(), ".repo-visualizer.json");
    if (import_fs2.default.existsSync(configPath)) {
      const fileConfig = JSON.parse(import_fs2.default.readFileSync(configPath, "utf8"));
  ...
  const excludedPaths = excludedPathsString ? excludedPathsString.split(",").map((str) => str.trim()) : config.excluded_paths;
  const excludedGlobsString = core.getInput("excluded_globs");
  const excludedGlobs = excludedGlobsString ? excludedGlobsString.split(";") : config.excluded_globs;
  core.endGroup();
